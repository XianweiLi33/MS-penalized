...
mstep.pen <- function(datac, df.new, lambda, penalty, theta, theta.mle){
  ...
  df.ps <- lapply(1:nrow(df.new), function(i){
    df <- df.new[i, ]
    data.frame(id = df$id, d = as.numeric(df[,c("d11", "d12", "d13", "d21", "d22", "d23")]), piece = rep(1:3, 2), type = rep(1:2, each = 3),
  ...
  if (penalty == "glasso"){
    ...
    fit <- glm.fit(x= cbind(1,glm.x), y= as.numeric(df.ps$d), weights = rep(nrow(glm.x)/nrow(datac), nrow(glm.x)), family= quasipoisson(link = "log")
                   , offset= as.numeric(log(df.ps$s)))
    cf <- coef(fit)
    # transform into theta.new
    coef.cov <- cf[-c(1:6)]
    current.beta1 <- coef.cov[1:100]
